    ...
    <div class="container mx-auto grid gap-4 lg:grid-cols-3 md:grid-cols-2 grid-cols-1">
        @foreach ($lists as $list)
            <div class="mt-5 py-5 bg-white font-semibold text-center rounded-3xl border shadow-lg mx-auto w-10/12 md:w-11/12">
                ...
                <button class="btn flex items-center mx-auto mt-5 transition duration-500 hover:scale-105 bg-indigo-600 hover:bg-indigo-800 px-4 py-2 text-gray-100 rounded-full" data-clipboard-text={{ env('APP_ENV') === 'local' ? 'http://127.0.0.1:8000/invitation/list?invitation_code=' . $list->invitation_code  : 'http://babybook.keanovancuyck.be/invitation/list?invitation_code=' . $list->invitation_code }}>
                    <span class="material-symbols-outlined mr-2">
                        content_copy
                    </span>
                    <p>{{ __('invite link') }}</p>
